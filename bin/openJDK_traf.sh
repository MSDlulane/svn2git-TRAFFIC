...
HOSTNAME=$(hostname).za.mhgad.com
TRAFNAME=$1
...
#JAVA_HOME=/usr/lib/jvm/jre-1.6.0-sun.x86_64/bin/
JAVA_HOME=/usr/lib/jvm/jre-1.6.0/bin
...
cd "$TRAFDIR"
exec -a "$TRAFPROCNAME" $JAVA_HOME/java -server $TRAFMEMSETTINGS $TRAFSNMPSETTINGS $TRAFJMXSETTINGS -jar "$TRAFHOME/lib/$TRAFLIB/TRAF.jar" "$TRAFDIR/config.xml" >/dev/null 2>/dev/null &
